...
public class JDBCHelperTest {
	...
	public static void main(String[] args) {
		...
		// 测试普通的插入操作
//		jdbcHelper.executeUpdate("insert into test_user(name,age) values(?,?)", new Object[] { "李四", 28 });

		// 测试查询语句
//		final Map<String, Object> testUser = new HashMap<String, Object>();
		
		//设计一个内部接口QueryCallback
		//那么在执行查询语句的时候，就可以封装和指定自己的查询结果的处理逻辑
		//封装在一个内部接口的匿名内部内对象中，传入JDBCHelper的方法
...
//		System.out.println(testUser.get("name") + ":" + testUser.get("age"));
		...
		paramsList.add(new Object[] {"王二",30});
