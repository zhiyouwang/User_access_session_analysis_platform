...
import com.wang.sparkproject.test.MockData;
import com.wang.sparkproject.util.DateUtils;
import com.wang.sparkproject.util.NumberUtils;
import com.wang.sparkproject.util.ParamUtils;
import com.wang.sparkproject.util.StringUtils;
import com.wang.sparkproject.util.ValidUtils;
 ...
 * 
 * spark作业接收用户创建的任务:
 * J2EE平台在接收到用户创建的任务后，会将任务信息插入MySQL的task表中，任务参数以JSON格式封装在task_param字段中
 * 接着J2EE平台会执行我们的spark-submit shell脚本，并将taskid作为参数传递给spark-submit shell脚本
 * 脚本在执行时，是可以接收参数的，并会将接收的参数，传递给Spark作业的main函数，参数就封装在main函数的args数组中
 * 
 * 这是spark本身提供的特性
 * 
 * @author boyWang
...
public class UserVisitSessionAnalyzeSpark {
	...
	public static void main(String[] args) {
		...
		SparkConf conf = new SparkConf()
				...
				.registerKryoClasses(new Class[] {
						...
						IntList.class});
		/**
		 * 比如，获取top10热门品类功能中，二次排序，自定义一个Key
		...
		/**
		 * actionRDD，就是一个公共RDD 第一，要用actionRDD，获取到一个公共的sessionid为key的PairRDD
		 * 第二，actionRDD，用在了session聚合环节里面
		 * 
		 * sessionid为key的PaireRDD，是确定了，在后面要多次使用的
		 * 1，与通过筛选的sessionid进行join，获取通过筛选的session的明细数据
		 * 2，将这个RDD，直接传入aggregateBySession方法，进行session聚合统计（在聚合方面，与前面进行了相同的PairRDD操作）
		 * 
		 ...
		 * 
		 * 如果是persist(StorageLevel.MEMORY_ONLY()),纯内存，无序列化，那么就可以同cache（）方法来替代
		...
		// 到这里为止，获取的数据是<sessionid，（searchKeywords，clickCategoryIds，age，professional，city，sex）>
		JavaPairRDD<String, String> sessionid2AggrInfoRDD = aggregateBySession(sqlContext, sessionid2actionRDD);

		/*
		 * System.out.println(sessionid2AggrInfoRDD.count()); for (Tuple2<String,
		 * String> tuple : sessionid2AggrInfoRDD.take(10)) {
		 * System.out.println(tuple._2); }
		 */

		// 接着，就要针对session粒度的聚合数据，按照使用者指定的筛选参数进行数据过滤
		// 相当于自己编写的算子，是要访问外面的任务参数对象的
		// 所以，要记得，匿名内部内（算子函数），访问外部对象，是要给外部对象使用final修饰的
		...
		JavaPairRDD<String, Row> sessionid2detailRDD = getSessionid2detailRDD(filteredSessionid2AggrInfoRDD,
				sessionid2actionRDD);
		sessionid2detailRDD = sessionid2actionRDD.persist(StorageLevel.MEMORY_ONLY());

		/**
		 * 对于Accumulator这种分布式累加计算的变量的使用，有一个重要说明
		 * 
		 * 从Accumulator中，获取数据，插入数据库的时候，一定要，是在有某一个action操作以后再进行
		 * 
		...
		/**
		 * 特别说明，要将上一个功能的session聚合统计数据获取到，就必须是在一个action操作触发job之后
		 * 才能从Accumulator中获取数据，否则是获取不到数据的，因为没有job执行，Accumulator的值为空
		 * 所以，在这里，将随机抽取的功能的实现代码，放在session聚合统计功能的最终计算和写库之前
		 * 因为随机抽取功能中，有一个countByKey算子，是action操作，会触发job
		 */

		// 计算出各个范围的session占比，并写入MySQL
		calculateAndPersisAggrStat(sessionAggrStatAccumulator.value(), task.getTaskid());

		/**
		 * session聚合统计（统计出访问时长和访问步长，各个区间的session数量占总session数量的比例）
		 * 
		 * 如果不进行重构，直接来实现，思路： 1、actionRDD，映射成<session,Row>的格式
		 ...
		 * 
		 * 普通实现思路的问题： 1、为什么还要用actionRDD，去映射？其实我们之前在session聚合的时候，映射已经做过了。
		 * 2、是不是一定要，为了session的聚合这个功能，单独去遍历一遍session？其实没有必要，已经有session数据
		 * 之前过滤session的时候，其实，就相当于，是在遍历session，那么这里就没必要再去过滤一遍了
		 * 
		 * 重构实现思路： 1、不要去生成任何新的RDD（处理上亿的数据） 2、不要去单独遍历一遍session的数据（处理上千万的数据）
		 * 3、可以在进行session聚合的时候，就直接计算出来每个session的访问时长和访问步长
		 * 4、在进行过滤的时候，本来就要遍历所有的聚合session信息，此时，就可以在某个session通过筛选条件后
		 * 将其访问时长和访问步长，累加到自定义的Accumulator上面
		 * 5、就是两种截然不同的思考方式，和实现方法，在面对上亿，上千万数据的时候，甚至可以节省时间长达半个小时，或者数个小时
		 * 
		 * 在开发spark大型复杂项目时： 1、尽量少生成RDD 2、尽量少对RDD进行算子操作，如果有可能，尽量在一个算子里面，实现多个需要做的功能
		 * 3、尽量少对RDD进行shuffle算子操作，比如groupBykey，reduceBykey，sortBykey（map、mapToPair）
		 * shuffle操作，会导致大量的磁盘读写，严重降低性能 有shuffle的算子，和没有shuffle的算子，甚至性能，会达到几十分钟，甚至数个小时的差别
		 * 有shuffle的算子，很容易导致数据倾斜，一旦数据倾斜，性能就会大大影响
		 */
	...
	/**
	 * 获取SQLContext 如果是在本地测试坏境的话，那么就生成SQLContext对象 如果实在生产坏境运行的话，那么就生成HiveContext对象
	 * 
	...
	private static JavaPairRDD<String, Row> getSessionid2ActionRDD(JavaRDD<Row> actionRDD) {
		...
		return actionRDD.mapToPair(new PairFunction<Row, String, Row>() {
			...
			/**
			 * PairFunction 第一个参数，相当于是函数的输入 。第二个参数和第三个参数，相当于是函数的输出（Tuple），分别是Tuple第一个和第二个值
			 */
			...
			public Tuple2<String, Row> call(Row row) throws Exception {
				// row.getString(2)是表中对应的sessionId
	 ...
	 */
	private static JavaPairRDD<String, String> aggregateBySession(SQLContext sqlContext,
				...
				.mapToPair(new PairFunction<Tuple2<String, Iterable<Row>>, Long, String>() {
					...
					public Tuple2<Long, String> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
						...
						while (iterator.hasNext()) {
							...
							// 这里对数据进行说明一下，并不是每一行访问行为都有searchKeyword和clickCategoryId两个字段的
							// 其实，只有搜索行为，是有searchKeyword字段的
							// 只有点击行为，是有clickCategoryId字段的
						...
						// 如果是根用户信息进行聚合的话，那么key，就不应该是sessionid
						// 应该是userid，才能够跟<userid，row>格式的用户信息进行聚合
						// 如果我们这里直接返回<sessionid，partAggrInfo>，还得再做一次mapToPair算子
						// 将RDD映射成<userid，partAggrInfo>的格式，那么就多此一举

						// 所以，这里其实可以直接，返回的数据格式就是<userid，partAggrInfo>
						// 然后跟用户信息join的时候，将partAggrInfo关联上userInfo
						// 然后再直接将返回的tuple的key设置成sessionid
						// 最后的数据格式，还是<sessionid，fullAggrInfo>

						// 聚合数据，这里统一定义，使用key=value|key=value
	...
	private static JavaPairRDD<String, String> filterSessionAndAggrStat(
		...
		// 为了使用后面的ValieUtils,所以，首先将所有的筛选参数拼接成一个链接串
		// 是为了后面的性能优化
		...
		JavaPairRDD<String, String> filteredSessionid2AggrInfoRDD = sessionid2AggrInfoRDD
				.filter(new Function<Tuple2<String, String>, Boolean>() {
					...
					public Boolean call(Tuple2<String, String> tuple) throws Exception {
						...
						// 按照搜索词进行过滤
						// in这个校验方法，只有有任何一个，与筛选条件任何一个搜索词相同，即通过
	...
	private static void randomExtractSession(
		...
		/**
		 * 这里不要着急写大量代码，一定要多动脑子思考
		 * 每天每小时的session数量，然后计算出每天每小时的session抽取索引，遍历每天每小时session
		 * 首先抽取出的session的聚合数据，写入session_random_extract表 所以第一个RDD的value，应该是session聚合数据
		 */
		...
		// 如果发现某个session恰巧在我们制定的这天这小时的随机抽取索引上
		// 那么抽取该session,直接写入MySQL的random_extract_session表
		// 将抽取出来的session id返回回来，形成一个新的JavaRDD<String>
		// 然后最后一步，是用出去出来的sessionid，去join它们的访问行为明细数据，写入session表
		JavaPairRDD<String, String> extractSessionidsRDD = time2sessionRDD
				.flatMapToPair(new PairFlatMapFunction<Tuple2<String, Iterable<String>>, String, String>() {
					...
					public Iterable<Tuple2<String, String>> call(Tuple2<String, Iterable<String>> tuple)
						...
						ISessionRandomExtractDAO sessionRandomExtractDAO = DAOFactory.getSessionExtractDAO();
	 ...
	 * @param value
	 */
	private static void calculateAndPersisAggrStat(String value, long taskid) {
		// 从Accumulator统计串中获取值
	...
	private static List<Tuple2<CategorySortKey, String>> getTop10Category(long taskid,
		...
		/**
		 * 必须要进行去重 如果不去重的话，会出现重复的categoryid，排序会对重复的categoryid以及countInfo进行排序
		 * 最后很可能会拿到重复调度
		 */
		 ...
		 * join到自己的某个数据，比如点击或下单或者支付次数，那么该categoryidRDD还是要保留下来的 只不过，没有join到的那个数据，就是0了
		 */
		...
		JavaPairRDD<Long, Long> payCategoryIdRDD = payActionRDD
				.flatMapToPair(new PairFlatMapFunction<Tuple2<String, Row>, Long, Long>() {
					...
					public Iterable<Tuple2<Long, Long>> call(Tuple2<String, Row> tuple) throws Exception {
						Row row = tuple._2;
						String payCategoryIds = row.getString(10);
						String[] payCategoryIdsSpilted = payCategoryIds.split(",");

						List<Tuple2<Long, Long>> list = new ArrayList<Tuple2<Long, Long>>();

						for (String payCategoryId : payCategoryIdsSpilted) {
	...
	private static JavaPairRDD<Long, String> joinCategoryAndData(JavaPairRDD<Long, Long> categoryidRDD,
		...
		// 如果用leftOuterJoin，就可能出现，右边那个RDD，join过来时，没有值
					...
					public Iterable<Tuple2<String, String>> call(Tuple2<Long, Iterable<String>> tuple)
						...
						for (String sessionCount : top10Sessions) {
							if (sessionCount != null) {
								...
								ITop10CategorySessionDAO top10CategorySessionDAO = DAOFactory
										.getTopITop10CategorySessionDAO();
